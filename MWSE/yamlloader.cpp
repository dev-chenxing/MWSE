...
/*
	Adapted from https://gitlab.com/OpenMW/openmw/-/blob/master/components/lua/yamlloader.cpp under GPLv3
...
namespace OpenMW::LuaUtil
{
    ...
    {
        const auto yaml = YAML::LoadAll(input);
		return loadAll(yaml, lua);
    }
    ...
    namespace
    {
        ...
        [[noreturn]] void nodeError(const YAML::Node& node, const std::string& message)
        {
			...
			std::stringstream ss;
			ss << message << "  at line=" << mark.line + 1 << " column=" << mark.column + 1 << " position=" << mark.pos + 1;
